...
select *,sell_price*1.15 as "new price" from product_master where sell_price>1500;
...
select avg(sell_price)"Average price of products" from product_master;
...
select concat(day(dely_date)," ",monthname(dely_date)," ",year(dely_date))"Delivery date" from sale_order;

select avg(qty_ordered) from sale_order_details where 
select date_format(dely_date,"%d %M %y") from sale_order;

select date_add(now(), interval 15 day)"Date after 15 days";

select date_format(from_days(datediff(now(),dely_date)),"%d %m %y") from sale_order;

-----------------------------------------------------------------------

select count(s.product_no),m.description from sale_order_details as s,product_master as m where s.product_no=m.product_no  group by s.product_no;

select sum(m.sell_price),m.description from sale_order_details as s,product_master as m where s.product_no=m.product_no  group by s.product_no;

select cm.name,avg(sod.qty_ordered) from sale_order_details as sod,sale_order as so,client_master as cm where sod.order_no=so.order_no and so.client_no = cm.client_no group by cm.client_no having sum(sod.product_rate)>15000;


select sum(sod.product_rate) from sale_order_details as sod, sale_order as so where sod.order_no=so.order_no and so.billed_yn="y";
...
select pm.* from sale_order_details as sod,sale_order as so,client_master as cm,product_master as pm where sod.order_no=so.order_no and sod.product_no=pm.product_no and so.client_no=cm.client_no and cm.name="ivan bayross";

select pm.* from sale_order_details as sod,sale_order as so,product_master as pm where sod.order_no=so.order_no and sod.product_no=pm.product_no and month(so.dely_date)=month(now());

select cm.name from sale_order_details as sod,sale_order as so,client_master as cm,product_master as pm where sod.order_no=so.order_no and sod.product_no=pm.product_no and so.client_no=cm.client_no and pm.description="cd drive";

select sod.order_no,sod.product_no from sale_order_details as sod,sale_order as so,client_master as cm,product_master as pm where sod.order_no=so.order_no and sod.product_no=pm.product_no and so.client_no=cm.client_no and pm.description="1.44 floppies" and sod.qty_ordered<5;

select sod.product_no,sod.qty_ordered from sale_order_details as sod,sale_order as so,client_master as cm,product_master as pm where sod.order_no=so.order_no and so.client_no=cm.client_no and sod.product_no=pm.product_no and cm.name in ("ivan bayross","Vandana Saitwal");

select sod.product_no,sod.qty_ordered from sale_order_details as sod,sale_order as so,client_master as cm,product_master as pm where sod.order_no=so.order_no and so.client_no=cm.client_no and sod.product_no=pm.product_no and cm.client_no in ("c01","c02");
...
select product_no,description from product_master where product_no not in (select distinct product_no from sale_order_details);

select name,address1,address1,city,pincode from client_master where client_no in (select client_no from sale_order where order_no="O19001");

select name from client_master where client_no in (select distinct client_no from sale_order where order_date<'1996-05-01');


select client_no,name from client_master where client_no in(select client_no from sale_order where order_no in (select order_no from sale_order_details where product_no in (select distinct product_no from product_master where description="1.44 drive")));
